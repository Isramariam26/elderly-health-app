...
const PatientDashboard = ({ 
  ...
  return (
    <div className="dashboard-content-wrapper">
      ...
      {/* Pinned Patient Name, Age, Risks & Location */}
      <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center', margin: '20px 0 40px', gap: '16px'}}>
        <div style={{textAlign: 'center'}}>
          <h1 style={{fontSize: '2.5rem', color: 'var(--text-main)', margin: '0 0 8px 0'}}>
            {patient.name} {patient.age && <span style={{fontSize: '1.8rem', color: 'var(--text-muted)', fontWeight: 'normal'}}>, {patient.age} y/o</span>}
          </h1>
          {patient.risks && patient.risks.length > 0 && (
            <div style={{display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px', color: 'var(--status-critical)', fontSize: '1rem', fontWeight: '500'}}>
              <AlertTriangle size={18} />
              Risk Profile: {patient.risks.join(', ')}
            </div>
          )}
        </div>
