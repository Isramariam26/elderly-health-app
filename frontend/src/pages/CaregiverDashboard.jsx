          ...
          <div style={{display: 'flex', alignItems: 'center', gap: '16px'}}>
             ...
             <div>
               ...
               <p style={{margin: '8px 0 0 0', fontSize: '0.85rem', opacity: 0.8}}> 
                 {activeEmergency.assignmentReason || "Caretaker routing based on proximity and Registered Nurse level skills."}
               </p>
             </div>
          </div>
          ...
          <div className="modal-content" onClick={e => e.stopPropagation()}>
            ...
            <div className="modal-body">
              ...
              <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '24px'}}>
                ...
                <div>
                  <h3 style={{display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '16px'}}><Activity size={18} color="var(--accent-red)" /> Medical Profile & Contacts</h3>
                  <div className="task-list" style={{boxShadow: 'none', padding: '16px', display: 'flex', flexDirection: 'column', gap: '16px'}}>
                     <div>
                       <strong style={{color: 'var(--text-muted)', fontSize: '0.85rem', textTransform: 'uppercase'}}>History</strong>
                       <ul style={{margin: '4px 0 0 16px', padding: 0}}>
                         {(selectedPatient.history || []).map((h, i) => (
                           <li key={i} style={{marginBottom: '4px', color: 'var(--text-main)'}}>{h}</li>
                         ))}
                       </ul>
                     </div>
                     
                     {selectedPatient.risks && selectedPatient.risks.length > 0 && (
                       <div>
                         <strong style={{color: 'var(--text-muted)', fontSize: '0.85rem', textTransform: 'uppercase'}}>Known Risks</strong>
                         <ul style={{margin: '4px 0 0 16px', padding: 0}}>
                           {selectedPatient.risks.map((r, i) => (
                             <li key={i} style={{marginBottom: '4px', color: 'var(--accent-red)'}}>{r}</li>
                           ))}
                         </ul>
                       </div>
                     )}
                     ...
                     {selectedPatient.emergencyContact && (
                       <div style={{backgroundColor: '#f8f9fa', padding: '12px', borderRadius: '8px', border: '1px solid var(--border-color)'}}>
                         <strong style={{color: 'var(--text-muted)', fontSize: '0.85rem', textTransform: 'uppercase'}}>Emergency Contact</strong>
                         <div style={{marginTop: '4px', color: 'var(--text-main)'}}>
                           <strong>{selectedPatient.emergencyContact.name}</strong>
                           <br />{selectedPatient.emergencyContact.phone}
                         </div>
                       </div>
                     )}
                  </div>
